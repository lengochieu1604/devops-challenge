...
- Infrastructure architecture
    ...
    - 
      ![Diagrams-99Tech Nginx V1 0 0 drawio](https://github.com/user-attachments/assets/65552bf2-01b7-4404-93a9-9f5edff79a48)
...
| **Order to troubleshoot** | **Causes/scenario expect to encounter** | **Description** | **Recovery steps** | **Impact** |
| --- | --- | --- | --- | --- |
| **1** | **Traffic Overhead** | Sudden spike in user traffic (e.g., live stream, flash sale) | Scale VM resources if possible (Memory), enable rate-limiting, configure WAF/CDN | Prevents overload, ensures system stability but it can lead to the downtime for a NGINX service based on the HA property of the Cloud  scalability services |
| **2** | **NGINX LB - Logging Overhead** | Excessive logging enabled (debug mode) | Change `error_log` level to `warn` or `error` in `nginx.conf` | Reduces disk I/O and memory consumption |
...
| **3** | **System - Kernel Memory Leak** | Kernel not releasing memory properly | Check `slabinfo`, restart VM if necessary, apply kernel updates | Ensures proper memory management and system stability |
...
| **4** | **External Threats - DOS/DDOS** | Attackers flooding NGINX with requests | Implement rate limiting, configure WAF | Protects against service disruption and high memory usage |
